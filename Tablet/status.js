...
function progressUpdate() {
	if ($('playercontrol_active_play').style.display == 'block') {
		...
		_progressAt++;
		setProgressBarWidth();
		// do an ajax update every 5 seconds while playing
		if (inc == 10) {
		...
		{
			var m = Math.floor(_progressAt / 60);
			var s = Math.floor(_progressAt - (m * 60)) % 60;
			var vv = m + ":" + zeroPad(s, 2);
			refreshElement('elapsed', vv);	// ...
		}
...
function setProgressBarWidth() {
	if($("progressbar_overlay").style.display == 'none')
		return;
...
function playerButtonControl(playerRepeatOrShuffle, selected, param, noRequest) {
	...
	if (noRequest) {
		...
		$('playercontrol_prev').src = 'html/images/pl_prev.png';
		$('playercontrol_next').src = 'html/images/pl_next.png';
...
// Called from onClick on Now Playing Repeat control.
function repeatButtonControl(param) {
	var current = 0;
	var controls = ['off', 'song', 'playlist'];	
	var idx = 0;
	
	controls.each(function(thisControl) {
		if (window.getComputedStyle($('repeatcontrol_' + thisControl), "").display != 'none')
		{
			current = idx;
			return;
		}
	...
	controls.each(function(thisControl) {
		if(idx == newSetting)	
			$('repeatcontrol_' + thisControl).style.display = 'inline';
		else
			$('repeatcontrol_' + thisControl).style.display = 'none';
		idx++;
	});	

	param = param + '&P2=' + newSetting;
	
	getStatusData(param, refreshAll);
}

// Called from onClick on Now Playing Shuffle control.
function shuffleButtonControl(param) {
	var current = 0;
	var controls = ['off', 'song', 'album'];	
	var idx = 0;
	
	controls.each(function(thisControl) {
		if (window.getComputedStyle($('shufflecontrol_' + thisControl), "").display == 'block')
		{
			current = idx;
			return;
		}
	...
	controls.each(function(thisControl) {
		if(idx == newSetting)	
			$('shufflecontrol_' + thisControl).style.display = 'block';
		else
			$('shufflecontrol_' + thisControl).style.display = 'none';
		idx++;
	});	
...
function playerControl(selected, param) {
	...
	controls.each(function(thisControl) {
		var imgSrc = null;
		if (thisControl == selected) {
			imgSrc = imgStub + thisControl + '_active.png';
		} else {
			imgSrc = imgStub + thisControl + '.png';
		}
